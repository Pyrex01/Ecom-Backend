...
def doOrder(request):
    ...
    email = mail.send_mail("Confirmation for orders placed",f"hello {request.user.First_Name} the item you want to buy is successfully placed in order Item name--{item.Name}",EMAIL_HOST_USER,[request.user.Email],fail_silently=False)
